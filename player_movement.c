...
void	check_tile(t_game *game, int old_x, int old_y)
{
	t_list		*tmp;
	t_object	*coin;
	t_spikes 	*spike;
	...
	while (tmp)
	{
		...
		if (spike->x == old_x && spike->y == old_y)
		{
			...
			{
				mlx_put_image_to_window(game->window.mlx, game->window.win,
					game->spikes_on.drawable_img, old_x * game->tile_size,
					old_y * game->tile_size);
			}
			else
			{
				mlx_put_image_to_window(game->window.mlx, game->window.win,
					game->spikes_off.drawable_img, old_x * game->tile_size,
					old_y * game->tile_size);
			}
		}
		tmp = tmp->next;
